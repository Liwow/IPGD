...
from deprecated import mip_utils
from deprecated.mip_utils import MPSolverResponseStatus
import scipy.sparse as sp
from pyscipopt import SCIP_STATUS, SCIP_PARAMSETTING
...
class Solver(abc.ABC):
  ...
  """
    ...
    def load_model(self, scip_mip) -> SolverState:
        ...
        elif type(scip_mip) is list:
            self.scip_mip = scip_mip[0]
        else:
            model = scip.Model()
            model.hideOutput(True)
            model.readProblem(scip_mip)
    ...
    def extract_lp_features_at_root(self) -> Dict[str, Any]:
        ...
        features['variable_features'] = torch.tensor(variable_features['values'], dtype=torch.float32)
        features['binary_variable_indices'] = torch.tensor(FeatureExtractor.extract_binary_indices(scip_mip),
                                                           dtype=torch.int64)
        features['model_maximize'] = torch.tensor(1.0 if scip_mip.getObjectiveSense() == 'maximize' else 0.0,
                                                  dtype=torch.float32)
        features['variable_names'] = FeatureExtractor.extract_name_feature(scip_mip)
        features['constraint_features'] = torch.tensor(constraint_features['values'], dtype=torch.float32)
        features['best_solution_labels'] = torch.tensor(0, dtype=torch.float32)
        features['variable_lbs'] = torch.tensor(FeatureExtractor.extract_lower_bounds(scip_mip), dtype=torch.float32)

        features['edge_index'] = torch.tensor(edge_indices, dtype=torch.int64)
        # features['edge_index'] = torch.tensor(FeatureExtractor.extract_edge_indices(edge_indices), dtype=torch.int64)

        features['all_integer_variable_indices'] = torch.tensor(FeatureExtractor.extract_integer_indices(scip_mip),
                                                                dtype=torch.int64)
        # 对于字符串特征的处理可能需要特别注意，因为 PyTorch 主要处理数值型数据
        features['edge_features_names'] = "coef_normalized"
        features[
            'variable_feature_names'] = "Variable features: (age, avg_inc_val, basis_status_0, basis_status_1, " \
                                        "basis_status_2, basis_status_3, coef_normalized, has_lb, has_ub, inc_val, " \
                                        "reduced_cost, sol_frac, sol_is_at_lb, sol_is_at_ub, sol_val, type_0, type_1, " \
                                        "type_2, type_3) "
        features['constraint_feature_names'] = "age, bias, dualsol_val_normalized, is_tight, obj_cosine_similarity"
        features['variable_ubs'] = torch.tensor(FeatureExtractor.extract_upper_bounds(scip_mip), dtype=torch.float32)
        features['edge_attr'] = torch.tensor(edge_features['values'], dtype=torch.float32)
        features['n_vars'] = torch.tensor(scip_mip.getNVars())
        features['n_int_vars'] = torch.tensor(scip_mip.getNIntVars())
