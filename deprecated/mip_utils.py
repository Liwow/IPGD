...
def convert_pyscipopt_solution_to_MPResponse(model, pyscip_solution,
                                             status: MPSolverResponseStatus) -> MPSolutionResponse:
...
def convert_mip_to_pyscipmodel(mip_model):
    ...
    for constraint_data in mip_model.constraint:
        ...
        model.addCons(constraint_data.lower_bound <= (lin_expr <= constraint_data.upper_bound),
                      name=str(constraint_data.name))
    return model
